...
RETURNING id, resource_id, user_id, starts_at, ends_at, note, state, ended_by, created_at, updated_at
`

type BumpReservationParams struct {
	AdminUserID sql.NullInt64 `json:"admin_user_id"`
	ID          int64         `json:"id"`
}

// Admin force-end of any active reservation.
func (q *Queries) BumpReservation(ctx context.Context, arg BumpReservationParams) (Reservation, error) {
	row := q.db.QueryRowContext(ctx, bumpReservation, arg.AdminUserID, arg.ID)
	var i Reservation
	err := row.Scan(
		&i.ID,
		&i.ResourceID,
...
const cancelOwnReservation = `-- name: CancelOwnReservation :one
UPDATE reservations SET
    state = 'cancelled',
    ended_by = ?1,
    updated_at = CURRENT_TIMESTAMP
WHERE id = ?2 AND user_id = ?1 AND state = 'active'
RETURNING id, resource_id, user_id, starts_at, ends_at, note, state, ended_by, created_at, updated_at
`

type CancelOwnReservationParams struct {
	UserID sql.NullInt64 `json:"user_id"`
	ID     int64         `json:"id"`
}

func (q *Queries) CancelOwnReservation(ctx context.Context, arg CancelOwnReservationParams) (Reservation, error) {
	row := q.db.QueryRowContext(ctx, cancelOwnReservation, arg.UserID, arg.ID)
	var i Reservation
	err := row.Scan(
		&i.ID,
		&i.ResourceID,
...
SELECT ?1, ?2, ?3, ?4, ?5
WHERE EXISTS (
    SELECT 1 FROM resources WHERE id = ?1 AND state = 'available'
)
AND NOT EXISTS (
...
SELECT id, slug, name, description, state, blocked_reason, capabilities, created_at, updated_at FROM resources WHERE id = ? LIMIT 1
`
...
WHERE r.state = 'active' AND r.ends_at > ?1
...
func (q *Queries) ListActiveReservations(ctx context.Context, now string) ([]ListActiveReservationsRow, error) {
	rows, err := q.db.QueryContext(ctx, listActiveReservations, now)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateOwnReservationEnd = `-- name: UpdateOwnReservationEnd :one
UPDATE reservations SET
    ends_at = ?1,
    updated_at = CURRENT_TIMESTAMP
WHERE reservations.id = ?2
  AND reservations.user_id = ?3
  AND reservations.state = 'active'
  AND ?1 > reservations.starts_at
  AND NOT EXISTS (
      ...
      WHERE other.resource_id = reservations.resource_id
        ...
        AND other.starts_at < ?1
        AND other.ends_at > reservations.starts_at
  )
RETURNING id, resource_id, user_id, starts_at, ends_at, note, state, ended_by, created_at, updated_at
`

type UpdateOwnReservationEndParams struct {
	EndsAt string `json:"ends_at"`
	ID     int64  `json:"id"`
	UserID int64  `json:"user_id"`
}
...
func (q *Queries) UpdateOwnReservationEnd(ctx context.Context, arg UpdateOwnReservationEndParams) (Reservation, error) {
	row := q.db.QueryRowContext(ctx, updateOwnReservationEnd, arg.EndsAt, arg.ID, arg.UserID)
	var i Reservation
	err := row.Scan(
		&i.ID,
		&i.ResourceID,
